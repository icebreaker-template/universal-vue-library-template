...
const rectangleElementInlineStyle =
  'position: absolute;pointer-events: none;border: 1px solid rgb(45, 140, 240);background: rgba(45, 140, 240, 0.2);display:none;'
...
class MouseSelection {
  ...
  private _selectStart = (event: MouseEvent) => {
    ...
    const x =
      (event.pageX + this.wrapDOM?.scrollLeft - window.pageXOffset) / this.scale
    const y =
    ...
    const callback: ((event: MouseEvent) => void) | undefined =
  ...
  private _selecting = (event: MouseEvent) => {
    ...
    const callback: ((event: RefitedMouseEvent) => void) | undefined =
